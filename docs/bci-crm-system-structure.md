...
> **当前阶段**：API Router Server v0.2（模块化 APIRouter）已就绪；本地可用 SQLite，生产接 5433
...
### 7.2 已实现接口（API Router Server v0.2）
...
| GET/POST | `/clients` | 客户列表 / 新建（撞单检测） |
| GET | `/clients/{id}` | 客户详情 |
| GET | `/clients/{id}/timeline` | 沟通 + 线索时间线 |
| GET/POST | `/interactions` | 沟通记录 |
| GET | `/leads` | 项目线索列表 |
| GET/POST | `/wecom/callback` | 企微回调 stub |
| GET | `/internal/clients` | Brain 内网调用（Token） |
| POST | `/ai/ask` | 权限内检索问答 stub |
| GET | `/docs` | Swagger UI |
...
| 路径 | 状态 | 下一步 |
|------|------|--------|
| `/wecom/callback` | stub | Token / AES 验签 + 指令解析 |
| `/ai/ask` | 本地检索 stub | 转发 Hindsight :8888，权限不变 |
| `/internal/clients` | Token 已启用 | 生产强随机 Token + 仅环回 |
| 权限 | 应用层角色过滤 | PostgreSQL RLS |
...
| 6 张核心表 + 8 用户 | ✅ |
| 与 Brain 实例隔离（B 方案） | ✅ |
| CRM API Router Server（仓库） | ✅ |
| 本机启动 API 8100 并验证 | ⬜ 待你本机执行 |
| 企微自建应用 | ⬜ |
| 腾讯云回调转发 | ⬜ |
| 客户/沟通 CRUD | ✅（API 层） |
| Brain internal 对接 | ✅（Token stub） |
| 权限 RLS / 字段级 | ⬜ |
